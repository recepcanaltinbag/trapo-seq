...
for line in data.splitlines():
    ...
    if line.startswith(">"):
        ...
        species, percentage = ', '.join(line.split(', ')[:-2]), float(line.split(', ')[-1])
...
# Teal ve turuncu renk paletleri
pastel_teal_cmap = create_pastel_teal_cmap()  # Teal tonları
pastel_orange_cmap = create_pastel_orange_cmap((1.0, 0.7, 0.3))  # Turuncu tonları
...
def plot_heatmaps(data_high, data_low):
    ...
    plt.tight_layout()
    plt.savefig("heatmaps_combinedv3.pdf")  # Grafik dosyaya kaydedilir
    plt.show()

plot_heatmaps(heatmap_data_high_long, heatmap_data_low_long)
...
combined_matrix_file_path = 'heatmap_data_combined_matrix.tsv'
