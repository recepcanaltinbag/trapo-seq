...
#INPUT: Folder of BAM FILEs, and threshold for filtering insertions
...
#Recep Can Altınbağ, 12 11 2024, v0.3
#-------------------------------------
...
def main_insert_finder_from_bam_batch(data_dir, insertion_threshold = 500):
    ...
    for folder in folders:
        print('\nProcessing: ', folder)
        output_file = os.path.join(current_dir, folder, output)
        bamfile = find_file_with_prefix(os.path.join(current_dir, folder), "03_", ".bam")
        if bamfile == None:
            print("No bam file in ", folder, ", skipping!")
            continue
        print('Corresponding bam: ', bamfile)
